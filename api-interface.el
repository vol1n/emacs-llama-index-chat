...
(defun llama-api-stream-request (endpoint method data callback)
...
Pass each response chunk to CALLBACK."
  (let ((url (concat "http://localhost:8000" endpoint))
    ...
    (url-retrieve url 'llama-handle-stream (list callback))))
...
(defun llama-handle-stream (status callback)
  ...
  (let ((response (buffer-substring-no-properties (point) (point-max))))
    (dolist (chunk (split-string response "\n" t))
      (funcall callback (json-read-from-string chunk))))
...
(defun llama-send-message (thread-id content callback)
  ...
  (llama-api-request "/thread/message" "POST" `(("thread_id" . ,thread-id) ("content" . ,content)) callback))
...
(defun llama-edit-message (thread-id message-id new-content callback)
  ...
  (llama-api-request "/thread/message" "PUT" `(("thread_id" . ,thread-id) ("message_id" . ,message-id) ("new_content" . ,new-content)) callback))
